...
if(isset($_SESSION['auth_key'])){
    if($_SESSION['auth_key']==2){
        $datatable_data="";
        $total=totaldiario();
        $transacciones=totaldiario_transacciones();
        $serial=ticketserialdiario();
        $query = "SELECT 
                    Tickets.id,
                    Usuarios.nombre,
                    ...
                    WHERE CURDATE()=DATE_FORMAT(Tickets.fechaticket,'%Y-%m-%e')
        ...
        <div class='input-group col-4'>
            <form method='POST' id='ticketserial'>
                ...
                <button onclick='imprimir($total,$transacciones)' class='btn btn-success form-control' name='imprimir_ticket_diario' type='submit'>Reporte Diario</button>
            </form>
        </div>
        ";
        $title_table="VENTA TOTAL DEL DIA: $$total";
        ...
        <div class='card shadow mb-4'>
            <div class='card'>
                <div class='card-body'>
                    <div class='row'>
                    <div class='input-group col-8'><p class='card-title'>$title_table</p></div>
                    $boton_imprimir
                    </div>
...
<script>
let table = new DataTable('#venta_diaria', {
    responsive: true,
    "pageLength": 25

});
$(document).ready( function () {$('#venta_diaria').DataTable();});
async function imprimir(total,transacciones){
  ...
  let lospesos = new Intl.NumberFormat('en-US', {
    style: 'currency',
    currency: 'USD',
});
